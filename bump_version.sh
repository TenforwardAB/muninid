...
# Abort immediately on any error so a failed `git add`/commit never falls through
# to tagging and pushing an incomplete release.
set -eo pipefail
...
if [ -f VERSION ]; then
    ...
    read
    echo -e "$PUSHING_MSG"
    git add CHANGELOG.md VERSION bump_version.sh
    ...
    if [ "$RESPONSE" = "y" ]; then
        ...
        read
        echo -e "$PUSHING_MSG"
        git add VERSION CHANGELOG.md bump_version.sh
